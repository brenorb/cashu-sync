...
Cashu Sync lets one user operate the same Silent Link Cashu wallet from multiple paired clients. V0 is deliberately narrow: one Nutshell mint, mint and melt only, one encrypted whole-wallet snapshot, and one minimal Nostr-compatible relay that atomically advances the snapshot head.
...
3. Wallets communicate directly with one configured [Nutshell](https://github.com/cashubtc/nutshell) mint for mint, melt, proof-state, and restore operations.
